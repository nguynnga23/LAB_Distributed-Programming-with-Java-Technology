...
@NamedQueries({ // JPQL
		...
		@NamedQuery(name = "Course.findByDepartment", query = "SELECT c FROM Course c WHERE c.department.id = :departmentID"),
		@NamedQuery(name = "Course.findCoursesWithMaxSumStudentEnrrollment", query = "select c from Course c where c.credits = (select max(credits) from Course)")})
